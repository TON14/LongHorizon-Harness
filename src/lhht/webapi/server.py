...
_WS_AUTH_MARKER = "lhht-auth.v1"
_WS_AUTH_TOKEN_PREFIX = "lhht-token."
...
def _configured_token(explicit: str | None) -> str | None:
    value = explicit if explicit is not None else os.environ.get("LHHT_WEB_TOKEN")
...
class StateRegistry:
    ...
    def _run_paths(self, run_id: str) -> tuple[Path, Path] | None:
        ...
        ``lhht``. Existing result/control children, however, must be
        ...
        try:
            while True:
                ...
                if fresh:
                    ...
                    # supervised and attached (`lhht run`) modes.
                    if idle_ticks % 4 == 0:
                        updated_snapshot = _snapshot_for(registry, state_for_run, run_id)
...
class WebServerHandle:
    """Handle for an in-process API used by ``lhht run --dashboard``."""
...
def start_web_server(
    *,
    ...
    thread = threading.Thread(target=server.run, name="lhht-web", daemon=True)
    thread.start()
