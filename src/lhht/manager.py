...
def _provider_retry_policy() -> _RetryPolicy:
    ...
    LHHT_PROVIDER_RETRY_{MAX_ATTEMPTS,BASE_SECONDS,CAP_SECONDS,MAX_TOTAL_SECONDS};
    set MAX_ATTEMPTS=0 to restore the old fail-fast behaviour.
    """
    return _RetryPolicy(
        max_attempts=int(_env_float("LHHT_PROVIDER_RETRY_MAX_ATTEMPTS", 8)),
        base_seconds=_env_float("LHHT_PROVIDER_RETRY_BASE_SECONDS", 60.0),
        cap_seconds=_env_float("LHHT_PROVIDER_RETRY_CAP_SECONDS", 900.0),
        max_total_seconds=_env_float("LHHT_PROVIDER_RETRY_MAX_TOTAL_SECONDS", 7200.0),
    )
...
def _read_local_bounded(path: Path, max_bytes: int, *, tail: bool = False) -> str | None:
    ...
    try:
        ...
        # the worker can also replace ``lhht``/``role_orchestration`` with a
...
def _write_terminal_failure(
    ...
    *,
    ...
    log_dir = Path(getattr(config, "log_dir", "./lhht"))
    role_dir = log_dir / "role_orchestration"
