...
def main(argv: list[str] | None = None) -> int:
    ...
    run_parser.add_argument(
        ...
        help="Override the log directory. Defaults to <runs-root>/<run-id>/lhht.",
    )
    ...
    run_parser.add_argument(
        ...
        default=os.environ.get("LHHT_WEB_TOKEN"),
    ...
    dash_parser.add_argument(
        "--auth-token",
        default=os.environ.get("LHHT_WEB_TOKEN"),
        help="Bearer token for remote dashboard/API access (also LHHT_WEB_TOKEN).",
    )
    ...
    web_parser.add_argument(
        "--auth-token",
        default=os.environ.get("LHHT_WEB_TOKEN"),
        help="Bearer token for remote/API access (also LHHT_WEB_TOKEN).",
    )
    ...
    for action, help_text in (
        ...
        sub_parser.add_argument(
            ...
            help="Plugin to set up. Run `lhht plugin list` for what each one provides.",
        )
        if action == "install":
            sub_parser.add_argument(
                "--agent",
    ...
    init_parser = add_command("init", "Generate ./.lhht/config.toml for this project")
...
def _doctor_node_toolchain() -> int:
    ...
    if not npm:
        _doctor_line(
            "WARN",
            "npm",
            "not found on PATH; `lhht plugin install` needs Node.js 20+ (https://nodejs.org)",
        )
        warnings += 1
    else:
...
def _doctor_plugin_state(*, codex_path: str | None) -> int:
    ...
    hint = "run `lhht plugin install {name}`"
    warnings = 0
...
def _init_command(args: argparse.Namespace) -> int:
    try:
        ...
        print("Use `lhht init --force` to replace it.", file=sys.stderr)
        return 1
...
def _broken_install_message(exc: ImportError) -> str:
    return (
        ...
        "Reinstall with `pip install --force-reinstall lhht`. "
        f"({exc})"
    )
...
def _run_command(args: argparse.Namespace, run_defaults: dict[str, object] | None = None) -> int:
    ...
    log_dir = str(Path(args.log_dir).expanduser() if args.log_dir else run_dir / "lhht")
    if getattr(args, "supervised", False):
