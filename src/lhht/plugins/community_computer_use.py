...
`~/.lhht/plugins/` -- never into `~/.codex/config.toml` or Claude's user
...
unrelated Codex/Claude session. Nothing here runs during `lhht run`;
setup is always an explicit `lhht plugin` opt-in.
"""
