...
1. **Агент по умолчанию — `codex`, его на машине НЕТ.** Без явного
   `agent = "zcode"` (конфиг) или `--agent zcode` (CLI) прогон падает сразу:
   `codex was not found`. Это самая частая причина «проблем в новой сессии».
...
|---|---|---|
| `codex was not found` при старте | не задан агент | `agent = "zcode"` в конфиге или `--agent zcode` |
